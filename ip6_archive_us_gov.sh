...
for i in {01..19}; do
  docker run -d --name archiveteam_$i --env-file=/tmp/env --publish $port:8001 --label=com.centurylinklabs.watchtower.enable=true --log-driver json-file --log-opt max-size=50m --restart=on-failure  atdr.meo.ws/archiveteam/usgovernment-grab --concurrent 1 penguaman
